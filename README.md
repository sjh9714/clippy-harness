...
A Windows 98 desktop skin + office assistant pet for the [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) Web UI.
...
- **Full retro reskin** — window chrome, title bars, taskbar, Start button, dialogs, the teal. Built on the DSH skin token layer.
...
- **Classic dialogs** — when a session crashes: *"Your agent has performed an illegal operation."*
- Optional classic sounds. One npm install.
...
为 DeepSeek Harness Web UI 打造的 Windows 98 复古皮肤 + 办公助手宠物：

- 完整复古换肤（基于 DSH 皮肤 token 层）
- 助手对真实智能体状态做出反应：思考、执行工具、完成、报错
- 经典对话框：会话崩溃时弹出 *"Your agent has performed an illegal operation."*
